...
app = FastAPI(
    title="News RS API", description="API for querying topics and news articles", version="1.0.0"
)
...
VALID_API_TOKENS = {
    os.getenv("API_ACCESS_TOKEN"): {"name": "Frontend App", "permissions": ["read", "write"]},
}
...
async def get_topics_by_date_range(
    ...
    if start > end:
        raise HTTPException(
            status_code=400, detail="'from' date must be before or equal to 'to' date"
        )
    ...
    try:
        ...
        with MongoClient(
            ...
            results = mongo_collection.find(
                filter={"topics_per_day": {"$elemMatch": {"date": {"$gte": start, "$lte": end}}}},
...
async def get_articles_by_topic(
    ...
    try:
        ...
        to_date = datetime.fromisoformat(to_date).replace(
            hour=23, minute=55, second=0, microsecond=0
        )
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date format. Use YYYY-MM-DD")
...
@app.post("/question")
async def news_rs_by_question(payload: QuestionPayload, token_data: dict = Depends(verify_token)):
    """Recommends articles based on input query"""
    rs = RecommenderSystem(vectorized_db=chroma_client)
    ...
    return {"summary": response.get("summary", ""), "articles": articles}


@app.get("/latest_news")
async def get_latest_news(token_data: dict = Depends(verify_token)):
    ...
    results = collection.get(where={"date": date}, limit=6)
    articles, enriched_metadata = parse_chroma_results(results)
    if enriched_metadata:
        collection.update(
            ids=list(enriched_metadata.keys()),

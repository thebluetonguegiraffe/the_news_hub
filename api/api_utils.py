...
def parse_chroma_results(chroma_results: Dict, reverse: bool = True) -> Union[List[Dict], Dict]:
    articles = []
    enriched_metadata = {}
    ...
    for i, (metadata, chroma_id) in enumerate(pairs):
        ...
        filtered_images, extra_image = filter_image_url_list(
            image_urls=metadata.get("image", "").split(" "), url=url
        )
        if extra_image:
...
def parse_dict_results(
    ...
    for i, item in enumerate(results):
        ...
        filtered_images, _ = filter_image_url_list(
            image_urls=item.get("image", "").split(" "), url=url
        )
        article["id"] = i
